...
public class Indexer {
  ...
  public Indexer(CANBus bus) {
    ...
    switch (Constants.currentMode) {
      ...
      case REAL -> {
        // Feeder (single motor)
        PhoenixTalonFX feederMotor =
            new PhoenixTalonFX(30, bus, "Feeder");

        feeder = new FeederRollerSubsystem(feederMotor);

        // Indexer rollers (right is primary, left follows)
        PhoenixTalonFX rightIndexer =
            new PhoenixTalonFX(31, bus, "IndexerRight");
        ...
        PhoenixTalonFX leftIndexer =
            new PhoenixTalonFX(
                ...
                bus,
                "IndexerLeft",
                new PhoenixTalonFollower(31, MotorAlignmentValue.Aligned));
        ...
        shotStagger =
            new ShotStagger(
                new HardwareDIO("ShotPathA", 3),
                new HardwareDIO("ShotPathB", 4));
      }

      case SIM -> {
        TalonFXSimpleMotorSim feederMotor =
            new TalonFXSimpleMotorSim(30, bus, "Feeder", 0.001, 1);
        ...
        shotStagger =
            new ShotStagger(
                SimDIO.fromNT("ShotPathA"),
                SimDIO.fromNT("ShotPathB"));
      }

      default -> {
        NoOppTalonFX feederMotor =
            new NoOppTalonFX("Feeder", 0);

        feeder = new FeederRollerSubsystem(feederMotor);

        NoOppTalonFX rightIndexer =
            new NoOppTalonFX("IndexerRight", 0);

        NoOppTalonFX leftIndexer =
            new NoOppTalonFX("IndexerLeft", 0);
  ...
  /**
   * This should be scheduled whenever the shooter is actively firing.
   * It runs feeder + rollers and lets the shot stagger alternate paths.
   */
  ...
  public Command stopAll() {
    return feeder.stop()
        .alongWith(indexerRollers.stop())
        .alongWith(shotStagger.stop());
  }
}
